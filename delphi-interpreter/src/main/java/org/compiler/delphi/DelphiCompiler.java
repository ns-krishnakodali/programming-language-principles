...
public class DelphiCompiler {
    ...
    private String compileProgram(DelphiParser.ProgramContext program) {
        reset();
        try {
            ...
            return compileTraceFallback(program, ex);
        }
    }

    private String compileTraceFallback(DelphiParser.ProgramContext program, CompilerException original) {
        reset();
        String output = executeWithInterpreter(program);
        String traceComment = "Generated by fallback trace backend after unsupported feature: "
                + original.getMessage();

        fmtStr = internCString("%s");
        String outputName = internCString(output);

        StringBuilder module = new StringBuilder();
        module.append("; ").append(traceComment).append('\n')
        ...
        module.append('\n')
                .append("define i32 @main() {\n")
                .append("entry:\n")
                .append("  call i32 (ptr, ...) @printf(ptr ")
                .append(pointerToGlobal(fmtStr))
                .append(", ptr ")
                .append(pointerToGlobal(outputName))
                .append(")\n")
                .append("  ret i32 0\n")
                .append("}\n");
    ...
    private String executeWithInterpreter(DelphiParser.ProgramContext program) {
        ...
        try {
            ...
            interpreter.setConstantPropagationLog(false);
